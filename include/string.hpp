...
namespace ms {
    ...
    public:
        ...
        string(const char* ps, size_t len) : _size(len) {
            ...
            _ptr = new char[_capacity];
            memset(_ptr, 0, _capacity);
            for (size_t i = 0; i < len; ++i) _ptr[i] = ps[i];
        }
        /**
        ...
        string(string& s) : _size(s.size()), _capacity(s.capacity()) {
            _ptr = new char[_capacity];
            for (size_t i = 0; i < _size; ++i) _ptr[i] = s[i];
        }
        /**
         * @brief 析构函数
         */
        ...
        string& operator=(const char* ps) {
            ...
            _ptr = new char[_capacity];
            memset(_ptr, 0, _capacity);
            for (size_t i = 0; i < _size; ++i) _ptr[i] = ps[i];
            return *this;
        }
        ...
        string& insert(size_t pos, char c) {
            if (full()) _extend();
            for (size_t i = _size; i > pos; --i) _ptr[i] = _ptr[i - 1];
            _ptr[pos] = c;
            return *this;
        }
        /**
        ...
        string& insert(size_t pos, const char* ps) {
            ...
            for (size_t i = _size; i >= pos; --i) _ptr[i + len] = _ptr[i];  // 字符向右移动len
            for (size_t i = 0; i < len; ++i) _ptr[i + pos] = ps[i];
            return *this;
        }
        ...
        string& erase(size_t pos, size_t length) {
            ...
            for (size_t i = pos; i < _size - length; ++i) _ptr[i] = _ptr[i + length];
