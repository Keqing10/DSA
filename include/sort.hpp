 ...
 * @brief 冒泡排序
 */
template <typename T> void bubbleSort(T *p, size_t len) {
 ...
 * @brief 快速排序
 */
template <typename T> void quickSort(T *p, size_t len) {
    if (len <= 1)
        return;
 ...
 * @brief 直接插入排序
 */
template <typename T> void insertSort(T *p, size_t len) {
 ...
 * @brief 希尔排序
 */
template <typename T> void shellSort(T *p, size_t len) {
    size_t gap = len / 2;
    while (gap >= 1) {
 ...
 * @brief 简单选择排序
 */
template <typename T> void checkSort(T *p, size_t len) {
 ...
 * @brief 堆排序
 */
template <typename T> void heapSort(T *p, size_t len) {
    heap<T> hp(p, len);
    hp.create();
 ...
 * @brief 归并排序
 */
template <typename T> void mergeSort(T *p, size_t len) {
    size_t *pa = new T[len];
    _merge(p, pa, 0, len - 1);
    delete[] pa;
}
 ...
 */
template <typename T> void _merge(T *p, T *pa, size_t le, size_t ri) {
    if (ri - le < 2) {
        if (p[ri] < p[le])
            swap(p[le], p[ri]);
        return;
